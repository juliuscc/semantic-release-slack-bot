...
module.exports = async (pluginConfig, context) => {
  ...
  const failTemplate = pluginConfig.onFailTemplate
  ...
  if (failTemplate) {
    ...
    if (typeof failTemplate === 'object') {
      slackMessage = template(failTemplate, variables)
    } else if (typeof failTemplate === 'function') {
      slackMessage = failTemplate(variables)
    } else {
      logger.log('Invalid type for `onFailTemplate` must be object or function')
      return
    }
  } else {
